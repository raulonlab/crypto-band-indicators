...
class FngWeightedAverageStrategy(OrderLoggerStrategy):
    ...
    params = dict(
        ...
        # weighted_buy_amount multiplier by fng index
...
class FngRebalanceStrategy(OrderLoggerStrategy):
    ...
    params = dict(
        min_order_period=7,  # Number of days between buys
        # position percents by Fng index
        rebalance_percents=[83.3, 66.6, 50, 33.3, 16.6],
    )

    def __init__(self):
        # Indicators
        self.fng_value = self.data1.close
        self.fng_ma = bt.indicators.WeightedMovingAverage(
            self.data1, period=self.params.min_order_period, subplot=False)
    ...
    def nextstart(self):
        ...
        self.rebalance(
            self.params.rebalance_percents[fng_info.get('fng_index')])
    ...
    def next(self):
        ...
        last_bar_executed_fng_info = FngIndicator._get_fng_value_details(
            int(self.fng_value[last_bar_executed_ago]))
        ...
        if fng_info.get('fng_index') == fng_ma_info.get('fng_index') and fng_info.get('fng_index') != last_bar_executed_fng_info.get('fng_index'):
            ...
            self.debug(
                f"  ...skip: condition not fullfilled. Current FnG: {fng_info['fng_ordinal']}, MA FnG: {fng_ma_info['fng_ordinal']},  Previous FnG: {last_bar_executed_fng_info['fng_ordinal']}")
