...
RAINBOW_BANDS_COLORS = ['#ff1716', '#e76a5e', '#ff852a',
                        '#fec68b', '#fff48b', '#b0e072', '#63ce9c', '#36b1c6', '#276feb']
...
class RwaIndicator:
    def __init__(self, indicator_start_date: Union[str, date, datetime, None] = None, ticker_symbol: str = 'BTCUSDT', binance_api_key: str = '', binance_secret_key: str = ''):
        ...
        # load data
        self.indicator_data = datas.get_nasdaq_ticker_time_series(
        ...
        popt, pcov = curve_fit(RwaIndicator._rwa_logarithmic_function, xdata, ydata, p0=[
                               10, 100, 90])  # p0 is justa guess, doesn't matter as far as I know
        ...
        for i in range(-2, 6):
            self.indicator_data[f"fitted_data{i}"] = np.exp(
                self.fittedYData + i * .455)

    def get_current_rainbow_band_index(self):
        # Get current ticker price from Binance
        price_dict = datas.get_binance_ticker_market_price(
            self.binance_api_key, self.binance_secret_key, ticker_symbol=self.ticker_symbol)
    ...
    def get_rainbow_band_index(self, price: float, at_date: Union[str, date, datetime, None] = None):
        ...
        if at_date:
            ...
            if (len(at_date_index_list) == 0):
                print(
                    f"[warn] RwaIndicator.get_rainbow_band_index: Data not found at date {at_date_value}")
                return None
    ...
    def plot_rainbow(self):
        ...
        plt.semilogy(
            self.indicator_data["Date"], self.indicator_data["Value"], 'k-', linewidth=1)
        ...
        for i in range(-2, 6):
            self.indicator_data[f"fitted_data{i}"] = np.exp(
                self.fittedYData + i*.455)
            plt.plot(self.indicator_data["Date"], np.exp(
                self.fittedYData + i*.455), linewidth=1, markersize=0.5)
