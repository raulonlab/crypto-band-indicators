...
class DataLoader:
    ...
    def load_data(self, source: str, label: str = None) -> DataLoader:
        ...
        try:
            ...
            print(
                f"[warn] Error loading csv cache file: {NASDAQ_CSV_CACHE_PATH}: {repr(e)}")
            cached_data = None

        # Fetch API data
        if not self.only_cache:
            if (source == 'ticker_nasdaq'):
                missing_data = DataLoader.get_nasdaq_ticker_time_series(
                    start=missing_start_date)
            # elif (source == 'ticker/binance'):
            #     self.dataframe = DataLoader.get_binance_ticker_time_series()
            elif (source == 'fng'):
                missing_data = DataLoader.get_fng_time_series(
                    start=missing_start_date)
            elif (source == 'rainbow'):
                missing_data = DataLoader.get_nasdaq_ticker_time_series(
                    start=missing_start_date)
        ...
        self.dataframes[label] = DataFrameWrapper(
            source=source, dataframe=all_data)
    ...
    def to_dataframes(self, start: Union[str, date, datetime, None] = None, end: Union[str, date, datetime, None] = None) -> List(pd.DataFrame):
        ...
        for dataframe_wrapper in list(self.dataframes.values()):
            ...
            dataframes.append(DataLoader.filter_by_dates(
                dataframe_wrapper.dataframe, start, end))
    ...
    def write_cache(self, df: pd.DataFrame, source: str) -> None:
        df.to_csv(SOURCE_INFO[source]['cache_path'], sep=';',
                  date_format='%Y-%m-%d', index=True, index_label='date')
    ...
    def read_cache(self, source: str) -> Union[pd.DataFrame, None]:
        ...
        for numeric_column in source_info['numeric_columns']:
            cached_data[numeric_column] = pd.to_numeric(
                cached_data[numeric_column], errors='coerce')
            # Drop 0, np values
            # cached_data = cached_data[cached_data[numeric_column] > 0]

        # Set index
        cached_data.set_index(
            source_info['index_column'], drop=True, inplace=True)
