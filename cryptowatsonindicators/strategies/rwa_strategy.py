...
class RwaWeightedAverageStrategy(OrderLoggerStrategy):
    ...
    params = dict(
        ...
        weighted_multipliers=[0, 0.1, 0.2, 0.35, 0.5, 0.75, 1, 2.5, 3]    # order amount multipliers (weighted) for each index
    )
    ...
    def next(self):
        ...
        buy_dol_size = self.params.weighted_buy_amount * self.params.weighted_multipliers[rwa_info.get('band_index', 2)] 
...
class RwaRebalanceStrategy(OrderLoggerStrategy):
    ...
    def __init__(self):
        ...
        self.rwa_band_index = RwaIndicatorWrapper(
            self.data1)
        self.rwa_ma = bt.indicators.WeightedMovingAverage(self.rwa_band_index.l.band_index, period = self.params.min_order_period, subplot=False)
    ...
    def nextstart(self):
        ...
        self.rebalance(self.params.rebalance_percents[rwa_info.get('band_index')])
    ...
    def next(self):
        ...
        rwa_ma_info = RwaIndicator._get_rainbow_info_by_index(int(self.rwa_ma[0]))
        # print(f"ma: value: {self.rwa_ma[0]:<3}, index: {rwa_ma_info.get('band_index')}")
        last_bar_executed_rwa_info = RwaIndicator._get_rainbow_info_by_index(int(self.rwa_band_index[last_bar_executed_ago]))
        ...
        if rwa_info.get('band_index') == rwa_ma_info.get('band_index') and rwa_info.get('band_index') != last_bar_executed_rwa_info.get('band_index'):
            ...
            self.debug(f"  ...skip: condition not fullfilled. Current Band: {rwa_info['band_ordinal']}, MA Band: {rwa_ma_info['band_ordinal']},  Previous Band: {last_bar_executed_rwa_info['band_ordinal']}")
